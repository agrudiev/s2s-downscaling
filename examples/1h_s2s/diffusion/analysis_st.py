...
def run_analysis(s2s_path, sim_path, cpc_path, time_idxs, num_samples):
    ...
    for time_idx in time_idxs:
        ...
        lead_idx = 2
        samples = images[lead_idx, :, time_idx, :, :]  # (ensemble/samples, lat, lon)

        # Initialize arrays, titles, and extents for plotting
        arrays = np.stack([samples[i] for i in range(num_samples)], axis=0)  # shape: (num_samples, lat, lon)

        # Plot maps
        fig, _ = plot_maps(arrays, titles, extents)
        script_dir = os.path.dirname(os.path.realpath(__file__))
        figs_dir = os.path.join(script_dir, "figs_st")
        os.makedirs(figs_dir, exist_ok=True)
        fig.savefig(os.path.join(figs_dir, f"ensembles_t{time_idx}.png"))
    
    # Plot CDFs
    arrays = [images[0, i] for i in range(num_samples)]
...
def main():
    ...
    sim_path = os.path.join(simulations_dir, "diffusion/light_longer_st_cli100_ens4.h5")
    s2s_path = os.path.join(test_data_dir, "det_s2s_nearest_low-pass_1h.h5")
    cpc_path = os.path.join(test_data_dir,"cpc_condtday1h.h5")

    # time_idxs = [i for i in range(8)]
    # time_idxs = [i for i in range(8, 16)]
